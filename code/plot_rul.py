...
ANOM_COLS   = ["n_if_anomalies", "n_cusum_soh", "n_cusum_cycle_soh",
               "n_cusum_epk", "n_cusum_heat", "n_cusum_spread"]
ANOM_LABELS = ["Isolation Forest", "CUSUM SoH", "CUSUM Cycle SoH",
               "CUSUM Energy/km", "CUSUM Heat", "CUSUM Cell Spread"]
ANOM_COLORS = ["#e41a1c", "#377eb8", "#17becf", "#ff7f00", "#4daf4a", "#984ea3"]

SLOPE_COLS   = ["soh_health_norm", "cycle_soh_slope_norm", "vsag_slope_norm",
                "ir_slope_norm", "energy_slope_norm", "heat_slope_norm", "spread_slope_norm"]
SLOPE_LABELS = ["SoH Health\n(EKF)", "Cycle SoH\nTrend", "Voltage Sag",
                "Internal\nResistance", "Energy/km", "Heat Rate", "Cell Spread"]
...
def load_data():
    ...
    # ── Compatibility shims: map new column names to legacy names used by plot fns ──
    rul["rul_years_exp_day"]      = rul["rul_days"] / 365.25
    rul["rul_years_recommended"]  = rul["bayes_rul_days"].fillna(rul["rul_days"]) / 365.25
    rul["fit_quality"]            = rul["rul_reliability"]
    rul["data_span_days"]         = (
        pd.to_datetime(rul["last_date"]) - pd.to_datetime(rul["first_date"])
    ).dt.days.fillna(0).astype(int)
    # exp_k_day_blended not generated by new soh_rul; approximate from soh slope
    # (just used for annotation in fig3 — use 0 if missing)
    if "exp_k_day_blended" not in rul.columns:
        rul["exp_k_day_blended"] = 0.0
...
# ── Fig 3: OLS SoH Trend Fits ─────────────────────────────────────────────────
def fig3_exp_fits(trends, rul):
    """Plot linear OLS SoH trendlines for the 6 most degradation-signal vehicles."""
    plt.style.use(STYLE)
    ...
    for idx, row in top6.iterrows():
        ...
        slope     = row.get("soh_slope_%per_day", 0.0) or 0.0
        rul_yr    = row["rul_years_exp_day"]   # pre-computed shim (rul_days/365.25)
        fit_qual  = row["fit_quality"]          # pre-computed shim (rul_reliability)
        span_days = row["data_span_days"]
        cur_soh   = row.get("current_soh", 98.0) or 98.0
        r2        = row.get("soh_r2", float("nan"))
        ...
        # linear trendline extended to EOL (or 5 years out if slope near 0)
        soh_at_0 = cur_soh - slope * span_days   # back-project to day 0
        if slope < -1e-6:
            t_eol = (soh_at_0 - EOL_SOH) / (-slope)
        else:
            t_eol = span_days + 365 * 5

        t_max = max(t_eol * 1.05, span_days + 200)
        t_fit = np.linspace(0, t_max, 300)
        soh_fit = soh_at_0 + slope * t_fit

        r2_lbl = f", R²={r2:.2f}" if not math.isnan(r2) else ""
        ax.plot(t_fit, soh_fit, color="#e74c3c", linewidth=1.8,
                label=f"slope={slope:.4f} %/day{r2_lbl}", zorder=4)
    ...
    fig.suptitle(
        "OLS SoH Trend Fits -- Top-6 Degradation-Signal Vehicles",
...
def fig7_composite_ranking(rul):
    ...
    component_styles = [
        ...
        ("cycle_soh_slope_norm",  "Cycle SoH",  "*", "#17becf"),
...
def fig18_cusum_heatmap(rul):
    ...
    CUSUM_COLS   = ["n_cusum_soh", "n_cusum_cycle_soh", "n_cusum_epk",
                    "n_cusum_heat", "n_cusum_spread"]
    CUSUM_LABELS = ["SoH (BMS)", "Cycle SoH", "Energy/km", "Heat Rate", "Cell Spread"]

    df = rul.copy()
    df["short_id"] = df["registration_number"].apply(short_id)
    # Only keep CUSUM cols that exist in this rul file (handles older artifacts gracefully)
    avail_cusum = [(c, l) for c, l in zip(CUSUM_COLS, CUSUM_LABELS) if c in df.columns]
    if not avail_cusum:
        print("  [fig18] No CUSUM columns found — skipping.")
        return None
    avail_cols, avail_labels = zip(*avail_cusum)
    avail_cols, avail_labels = list(avail_cols), list(avail_labels)
    df["total_cusum"] = df[avail_cols].fillna(0).sum(axis=1)
    df = df.sort_values("total_cusum", ascending=False).reset_index(drop=True)

    heat_df = df[avail_cols].fillna(0).copy()
    heat_df.columns = avail_labels
    heat_df.index   = df["short_id"]
...
def fig20_ekf_soh_trace(ekf: pd.DataFrame, rul: pd.DataFrame, n_vehicles: int = 9):
    ...
    for idx, reg in enumerate(plot_regs):
        ...
        # cycle_soh observations (primary SoH observation in new EKF; replaces capacity_soh)
        if "cycle_soh_obs" in vdf.columns:
            csoh_mask = vdf["cycle_soh_obs"].notna()
            if csoh_mask.any():
                ax.scatter(vdf.loc[csoh_mask, "date"], vdf.loc[csoh_mask, "cycle_soh_obs"],
                           s=20, color="#17becf", alpha=0.75, zorder=4,
                           marker="^", label="cycle_soh (obs)")

        # BMS SoH observations
        if "bms_soh_obs" in vdf.columns:
            bms_mask = vdf["bms_soh_obs"].notna()
            if bms_mask.any():
                ax.scatter(vdf.loc[bms_mask, "date"], vdf.loc[bms_mask, "bms_soh_obs"],
                           s=10, color="#aec7e8", alpha=0.45, zorder=3,
                           marker="o", label="BMS SoH (obs)")
        ...
        # Annotate current EKF SoH + RUL + thermal drift
        last = vdf.iloc[-1]
        ann_text = f"EKF: {last['ekf_soh']:.1f}%"
        if pd.notna(last.get("ekf_rul_days")):
            ann_text += f"\nRUL: {int(last['ekf_rul_days'])} days"
        if "ekf_temp_drift" in vdf.columns and pd.notna(last.get("ekf_temp_drift")):
            ann_text += f"\nΔT: {last['ekf_temp_drift']:+.3f}°C/hr"
...
# ── Fig 21: cycle_soh Fleet Trajectories ──────────────────────────────────────
def fig21_cycle_soh_fleet(trends, rul):
    """
    Fleet-wide cycle_soh trajectories (one line per vehicle, coloured by final
    cycle_soh value).  cycle_soh is the new per-cycle SoH estimate available for
    all session types; overlay with BMS soh_smooth for comparison.
    """
    if "cycle_soh" not in trends.columns:
        print("  [fig21] cycle_soh not in soh_trends.csv — skipping.")
        return None
    ...
    soh_map = rul.set_index("registration_number")["current_soh"].to_dict()
    vehicles = trends["registration_number"].unique()
    soh_vals = np.array([soh_map.get(v, 98.0) for v in vehicles], dtype=float)
    norm     = plt.Normalize(vmin=np.nanmin(soh_vals), vmax=np.nanmax(soh_vals))
    cmap     = plt.cm.viridis

    for veh in vehicles:
        vdf    = trends[trends["registration_number"] == veh].sort_values("date_days")
        colour = cmap(norm(soh_map.get(veh, 98.0)))

        # Left panel: cycle_soh
        csoh = vdf["cycle_soh"].dropna()
        if len(csoh) >= 2:
            axes[0].plot(vdf.loc[csoh.index, "date_days"], csoh,
                         color=colour, linewidth=0.8, alpha=0.65)

        # Right panel: soh_smooth (BMS) for comparison
        if "soh_smooth" in vdf.columns:
            axes[1].plot(vdf["date_days"], vdf["soh_smooth"],
                         color=colour, linewidth=0.8, alpha=0.65)

    for ax, title in zip(axes, ["cycle_soh (per-cycle estimate)", "soh_smooth (BMS)"]):
        ax.axhline(EOL_SOH, color="red", linestyle="--", linewidth=1.5,
                   label=f"EOL ({EOL_SOH:.0f}%)")
        ax.set_xlabel("Days since first record", fontsize=11)
    ...
    cbar.set_label("Current BMS SoH (%)", fontsize=11)

    fig.suptitle(
        "Fleet SoH Trajectories — cycle_soh vs BMS soh_smooth",
        fontsize=14, fontweight="bold"
    )
    fig.tight_layout()
    path = os.path.join(PLOTS_DIR, "fig21_cycle_soh_fleet.png")
    fig.savefig(path, dpi=DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {path}")
    return fig
...
# ── Fig 22: cycle_soh vs EKF SoH per Vehicle (top 9) ─────────────────────────
def fig22_cycle_soh_vs_ekf(ekf: pd.DataFrame, trends: pd.DataFrame,
                            rul: pd.DataFrame, n_vehicles: int = 9):
    """
    Per-vehicle comparison: EKF SoH trace (filtered, physics-based) vs raw
    cycle_soh scatter (new column).  Useful for validating cycle_soh reliability.
    """
    if ekf is None:
        print("  [fig22] ekf_soh.csv not available — skipping.")
        return None
    if "cycle_soh" not in trends.columns and "cycle_soh_obs" not in ekf.columns:
        print("  [fig22] cycle_soh not found in trends or ekf — skipping.")
        return None
    ...
    plot_regs = [r for r in plot_regs if r in ekf["registration_number"].values]
    if not plot_regs:
        print("  [fig22] No matching vehicles — skipping.")
        return None
    ...
    for idx, reg in enumerate(plot_regs):
        ax     = axes_flat[idx]
        vekf   = ekf[ekf["registration_number"] == reg].sort_values("date")
        ...
        # EKF SoH trace
        ax.plot(vekf["date"], vekf["ekf_soh"],
                color="#2ca02c", linewidth=1.8, zorder=3, label="EKF SoH")
        if "ekf_soh_std" in vekf.columns:
            ax.fill_between(vekf["date"],
                            vekf["ekf_soh"] - vekf["ekf_soh_std"],
                            vekf["ekf_soh"] + vekf["ekf_soh_std"],
                            alpha=0.20, color="#2ca02c", zorder=2)

        # cycle_soh from ekf output (stored as cycle_soh_obs)
        if "cycle_soh_obs" in vekf.columns:
            csoh_mask = vekf["cycle_soh_obs"].notna()
            if csoh_mask.any():
                ax.scatter(vekf.loc[csoh_mask, "date"], vekf.loc[csoh_mask, "cycle_soh_obs"],
                           s=18, color="#17becf", alpha=0.75, zorder=4,
                           marker="^", label="cycle_soh (EKF obs)")

        # cycle_soh from trends (all discharge sessions)
        vtrd = trends[(trends["registration_number"] == reg) &
                      trends["cycle_soh"].notna()] if "cycle_soh" in trends.columns else pd.DataFrame()
        if not vtrd.empty and "date" in vtrd.columns:
            ax.scatter(vtrd["date"], vtrd["cycle_soh"],
                       s=10, color="#aec7e8", alpha=0.45, zorder=2, label="cycle_soh (disc)")
    ...
    fig.suptitle(
        "cycle_soh vs EKF SoH — Top Vehicles by Degradation Score\n"
        "(green = EKF filtered, cyan triangles = cycle_soh observations)",
        fontsize=13, fontweight="bold", y=1.01
    )
    fig.tight_layout()
    path = os.path.join(PLOTS_DIR, "fig22_cycle_soh_vs_ekf.png")
    fig.savefig(path, dpi=DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {path}")
    return fig
...
def main():
    ...
    # cycle_soh fleet trajectories (requires cycle_soh column in soh_trends.csv)
    fig21_cycle_soh_fleet(trends, rul)
    if os.path.exists(os.path.join(PLOTS_DIR, "fig21_cycle_soh_fleet.png")):
        fig_paths.append(os.path.join(PLOTS_DIR, "fig21_cycle_soh_fleet.png"))

    # cycle_soh vs EKF comparison per vehicle
    if ekf is not None:
        fig22_cycle_soh_vs_ekf(ekf, trends, rul)
        if os.path.exists(os.path.join(PLOTS_DIR, "fig22_cycle_soh_vs_ekf.png")):
            fig_paths.append(os.path.join(PLOTS_DIR, "fig22_cycle_soh_vs_ekf.png"))
